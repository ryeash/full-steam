...
public class LoneWolfManager extends AbstractGameStateManager {
    ...
    private void balanceTeams() {
        int huntersNeeded = (getMaxPlayers() - 1) - (int) entities.getPlayers().stream().filter(p -> p.getTeam() == 2).count();
        for (int i = 0; i < huntersNeeded; i++) {
            addAIPlayer(2); // Add AI to team 2
        }
        int wolvesNeeded = 1 - (int) entities.getPlayers().stream().filter(p -> p.getTeam() == 1).count();
        if (wolvesNeeded > 0) {
            // try to promote a hunter to be the wolf
            // preferentially move a human
            Player toMove = null;
            for (Player value : entities.getPlayers()) {
                if (value.getTeam() == 2) {
                    toMove = value;
    ...
    public PlayerSession addPlayer(long playerId, WebSocketSession channel) {
        ...
        if (loneWolfId == null) {
            ...
            session = super.addPlayer(playerId, channel, 1);
            applyLoneWolfStatus();
        } else {
            // Subsequent entities.getPlayers() are Hunters
            session = super.addPlayer(playerId, channel, 2);
        }
        return session;
    }
    ...
    protected void killPlayer(Player victim, Player shooter) {
        ...
        if (Objects.equals(victim.getId(), loneWolfId)) {
            loneWolfDeaths++;
            Player loneWolf = entities.getPlayer(loneWolfId);
    ...
    protected boolean checkEndConditions() {
        ...
        if (loneWolfId != null && !entities.getPlayerSessions().containsKey(loneWolfId)) {
        ...
        boolean allHuntersDown = entities.getPlayers()
                .stream()
        ...
        if (roundOver) {
            ...
            if (loneWolfId != null) {
                Player player = entities.getPlayer(loneWolfId);
                if (player != null) {
